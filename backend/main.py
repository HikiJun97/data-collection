...
@app.post("/valid", response_class=JSONResponse, response_model=None)
...
async def get_video(
    ...
    # token_payload: TokenInfo = Security(TokenHandler.verify_access_token),
    token_payload: Annotated[TokenInfo, Security(TokenHandler.verify_access_token)],
):
    # start_time_text: str = start_time.replace(":", "_")
    # end_time_text: str = end_time.replace(":", "_")
    filename: str = f"({user_id}){{{video_id}}}[{start_time}-{end_time}].mp4"
    video_file: Path = VIDEO_DIR / filename

    print(f"{filename} exists?", video_file.exists())
