...
async function getUserOptions(): Promise<Array<{id: string; validCount: string}>> {
	const usersData: Array<User> = JSON.parse(sessionStorage.getItem("users") || "[]");
...
function getDataOptions(targetUser: string): {
	...
	const users: Array<User> = JSON.parse(sessionStorage.getItem("users") || "[]");
...
async function initTomSelect(): Promise<void> {
	const userSelect = new TomSelect("#user-selection", {
		...
		options: [],
		render: {
			option: function (user: {id: string; validCount: string}, escape: (value: string) => string) {
				return `<div class="user-info"><div class="user-id">${escape(user.id)}</div><div class="valid-count">${escape(user.validCount)}</div></div>`;
			},
			item: function (user: {id: string; validCount: string}, escape: (value: string) => string) {
	...
	new TomSelect("#data-selection", {
		...
		options: [],
		render: {
			option: function (data: {id: string; validatedMark: string; videoId: string; timeRange: string}, escape: (value: string) => string) {
			...
			item: function (data: {id: string; validatedMark: string; videoId: string}, escape: (value: string) => string) {
...
async function initializeSelections(): Promise<void> {
	try {
		...
		const userSelect = (document.getElementById("user-selection") as HTMLSelectElement)?.tomselect;
...
function updateDataSelection(user: string): void {
	const dataSelect = (document.getElementById("data-selection") as HTMLSelectElement)?.tomselect;
