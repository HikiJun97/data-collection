...
from backend.CV2VideoCaptureContextManager import CV2VideoCaptureContextManager
from backend.utils import get_ydl_options
from backend.FaceDetector import FaceDetector
from backend.Validator import Validator
from backend.exceptions import VideoProcessingError
...
class VideoProcessor:
    ...
    def download_video(
        ...
        with yt_dlp.YoutubeDL(get_ydl_options(download_path, temp_path)) as ydl:
            ...
            info_dict: dict | None = ydl.extract_info(video_url, download=False)

            if info_dict is None:
                raise VideoProcessingError("info_dict is empty")

            # if info_dict is None, raise gr.Error
    ...
    def trim_video(
        ...
        output_video_path = output_path / f"{VID} [{start_time} - {end_time}].{EXT}"
    ...
    def get_media_info(self, video_path: Path):
        try:
            ...
            video_format = Config.codecs_and_formats.get(video_format, video_format)
